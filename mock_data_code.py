...
class MockCalcs:
    ...
    def get_BAO_mock(self):
        ...
        if self.settings_BAO['error_type']=='percentage':
            DH_error = 0.05 * DH
            DV_error = 0.05 * DV
            DM_error = 0.05 * DM
        else:
        ...
        DH_noisy = np.random.normal(DH, DH*0.02)
        DV_noisy = np.random.normal(DV, DV*0.02)
        DM_noisy = np.random.normal(DM, DM*0.02)
        ...
        np.save(self.settings_BAO['BAO_file_path']+'BAO_mock_data_DHDM.npy', data_BAO_DHDM)
        ...
        np.save('./mock_data/BAO_mock_data_DV.npy', data_BAO_DV)
    ...
    def get_SN_mock(self):
        ...
        if self.settings_SN['error_type'] == 'percentage':
            mB_error = 0.05 * mB
        else:
        ...
        np.save(self.settings_SN['SN_file_path']+'SN_mock_data.npy', data_SN)
    ...
    def get_GW_mock(self):
        ...
        if self.settings_GW['merger_rate'] == 'BNS':
            merger_rate = np.array([self.BNS_merger_rate(z) for z in z_calc])
        else:
            sys.exit('Unknown GW distribution: {}'.format(self.settings_GW['merger_rate']))
        ...
        if self.settings_GW['error_type'] == 'observational_error':
        ...
        np.save(self.settings_GW['GW_file_path']+'GW_mock_data.npy', data_GW)
